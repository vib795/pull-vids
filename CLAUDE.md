...
**The Homebrew formula is not in this repo.** It lives in the separate `vib795/homebrew-tap` repo and is regenerated by `update-homebrew.yml` on every release, so edit the workflow's heredoc, never the tap's `Formula/pull-vids.rb` directly.
